...
from alpaca_trade_api.common import get_base_url, get_data_stream_url, get_credentials, URL
from alpaca_trade_api.entity import Entity
from alpaca_trade_api.entity_v2 import quote_mapping_v2, trade_mapping_v2, bar_mapping_v2, Trade, Quote, Bar
...
class DataStream:
    def __init__(self, key_id: str, secret_key: str, base_url: URL, raw_data: bool, feed: str = 'iex'):
        self._key_id = key_id
    ...
    async def _connect(self):
        self._ws = await websockets.connect(self._endpoint, extra_headers={'Content-Type': 'application/msgpack'})
    ...
    async def _run_forever(self):
        ...
        while True:
            try:
                ...
                log.warning('websocket error, restarting connection: ' +
                         str(wse))
            finally:
                if not self._running:
                    break
...
class Stream:
    ...
    async def _run_forever(self):
        await asyncio.gather(self._trading_ws._run_forever(), self._data_ws._run_forever())
...
if __name__ == '__main__':
    ...
    ALPACA_API_KEY = keys[0].rstrip('\n')
    ALPACA_SECRET_KEY = keys[1].rstrip('\n')
    base_url = 'https://paper-api.alpaca.markets'
    data_url = 'https://data.alpaca.markets'

    conn = Stream(ALPACA_API_KEY, ALPACA_SECRET_KEY, URL(base_url))
