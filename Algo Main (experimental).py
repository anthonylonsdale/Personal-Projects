...
def options():
    ...
    try:
        ...
        pass
    formatting_excel_thread()
    time.sleep(1)
    options_calculations()


def formatting_excel_thread():
    pythoncom.CoInitialize()
    excel = win32.gencache.EnsureDispatch('Excel.Application')
    excel_id = pythoncom.CoMarshalInterThreadInterfaceInStream(pythoncom.IID_IDispatch, excel)
    thread = th.Thread(target=formatting_excel, kwargs={'excel_id': excel_id})
    thread.start()
    thread.join()


def formatting_excel(excel_id):
    pythoncom.CoInitialize()
    excel = win32.Dispatch(pythoncom.CoGetInterfaceAndReleaseStream(excel_id, pythoncom.IID_IDispatch))
    wb = excel.Workbooks.Open(r"C:\Users\fabio\PycharmProjects\AlgoTrader\Options Data.xlsx")
    for ticker_file_name in stock_tickers:
        wks = wb.Worksheets(ticker_file_name + ' Call Contracts')
        wks.Columns.AutoFit()
        wks = wb.Worksheets(ticker_file_name + ' Put Contracts')
        wks.Columns.AutoFit()
        wb.Save()
