...
def trade_settlement(activities_df, stock_tickers):
    ...
    return trade_book, short_trade_book, long_order_time_held, short_order_time_held, profit_per_symbol
...
def data_to_excel(data, sheet_name, cwd):
    path = fr'{cwd}\Daily Stock Analysis\Portfolio-Analysis\Analyzed Portfolio Data.xlsx'
    ...
    ExcelFormatting(path).formatting()
...
class portfolioAnalysis:
    ...
    def main(self):
        while True:
            try:
                ...
                url = f"{self.cwd}\\Daily Stock Analysis\\Portfolio-Analysis\\Portfolio Activities {date}.xlsx"
                with pd.ExcelWriter(url, engine='openpyxl') as writer:
                    activities_df.to_excel(writer, sheet_name=f'Activities')
                ExcelFormatting(url).formatting()
                ...
                try:
                    ...
                    prev_day_df = pd.DataFrame([activity._raw for activity in prev_day_activities])
                    prev_day_df = prev_day_df.iloc[::-1]
                    prev_day_df[['price', 'qty']] = prev_day_df[['price', 'qty']].apply(pd.to_numeric)
                    prev_day_df['net_qty'] = np.where(prev_day_df.side == 'buy', prev_day_df.qty, -prev_day_df.qty)
                    prev_day_df['net_trade'] = -prev_day_df.net_qty * prev_day_df.price
                    prev_day_df['cumulative_sum'] = prev_day_df.groupby('symbol')['net_qty'].apply(lambda h: h.cumsum())

                    url1 = f"{self.cwd}\\Daily Stock " \
                          f"Analysis\\Portfolio-Analysis\\Portfolio Activities {prev_day_date}.xlsx"
                    with pd.ExcelWriter(url1, engine='openpyxl') as writer:
                        prev_day_df.to_excel(writer, sheet_name=f'Activities')
                    ExcelFormatting(url1).formatting()

                    nonzero_trades = prev_day_df.groupby('symbol').filter(lambda trade: sum(trade.net_qty) != 0)
        ...
        self.date = date
        purchases_df = purchasing_filter(activities_df)
        t_b, s_t_b, l_o_t_h, s_o_t_h, pps = trade_settlement(purchases_df, self.stock_tickers)
        self.trade_calculations(t_b, s_t_b, l_o_t_h, s_o_t_h, pps)
    ...
    def trade_calculations(self, trade_book, short_trade_book, long_order_time_held, short_order_time_held,
        ...
        for stock in self.stock_tickers:
            ...
            market_returns_pct = round((quote_data[stock][0]['current price'] -
                                        float(init_data[stock][0]['open'])) / 100, 4)
        ...
        data_to_excel(data, sheet_name, self.cwd)
